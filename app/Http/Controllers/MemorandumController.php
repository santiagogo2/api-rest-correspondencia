...
class MemorandumController extends Controller
{
	...
	public function index(){
		$filed = Memorandum::with('AppUsers')
						   ->with('Dependence')
						   ->with('Documents')
						   ->with('Attached')
						   ->get();
	...
	public function show($id){
		...
		if(is_object($filed)){
			...
			$data = array(
				'status'			=> 'error',
				'code'				=> 400,
				'message'			=> 'No se han encontrado ningún Memorando con el ID '.$id
			);
		}
	...
	public function showByAppUsersId($userId){
		...
		if(is_object($filed)){
			...
			$data = array(
				'status'			=> 'error',
				'code'				=> 400,
				'message'			=> 'No se han encontrado ningún Memorando perteneciente al usuario con documento '.$userId	
			);
		}
	...
	public function showByAffair($affair){
		...
		if(is_object($filed)){
			...
			$data = array(
				'status'		=> 'error',
				'code'			=> 404,
				'message'		=> 'No se han encontrado ningún Radicado Interno cuyo asunto contenga '.$affair	
			);
		}
	...
	public function update($id, Request $request){
		...
		if(!empty($params_array)){
			...
			if($validate->fails()){
				...
				$filed = Memorandum::where('id', $id)->update($params_array);
